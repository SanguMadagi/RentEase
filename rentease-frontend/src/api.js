...
export const getAllProducts = async (userLat = null, userLon = null) => {
  try {
    let url = BASE_URL;
    if (userLat !== null && userLon !== null) url += `?userLat=${userLat}&userLon=${userLon}`;
...
export const addProduct = async (productData) => {
  try {
    const res = await fetch(BASE_URL, { method: 'POST', headers: getHeaders(), body: JSON.stringify(productData) });
...
export const updateProduct = async (id, productData) => {
  try {
    const res = await fetch(`${BASE_URL}/${id}`, { method: 'PUT', headers: getHeaders(), body: JSON.stringify(productData) });
...
export const getReviewsByProduct = async (productId) => {
  try {
    const res = await fetch(`${REVIEW_URL}/product/${productId}`, { headers: getHeaders() });
...
export const addReview = async ({ productId, rating, comment }) => {
  try {
    const res = await fetch(REVIEW_URL, { method: 'POST', headers: getHeaders(), body: JSON.stringify({ productId, rating, comment }) });
